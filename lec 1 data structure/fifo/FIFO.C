...
{
	if(buff == NULL)  //cheak if the buffer is full or not
		return FIFO_NULL;

	fifo->base = buff;              //make the base refre to the start of the arrray
	fifo->head = buff;              //make the head refre to the start of the arrray
	fifo->tail = buff;              //make the tail refre to the start of the arrray
	fifo->length = length;
	fifo->count = 0;
...
FIFO_BUFF_STATUS FIFO_Enqueue (FIFO_Buff_t* fifo, element_type item)
{
	...
	*(fifo->head) = item;
	...
	if(fifo->head == (fifo->base + (fifo->length * sizeof(element_type))))
		//or if(fifo->length == fifo->count) 
		fifo->head = fifo->base;   //retuen the head to the base if the head go  to the end of the array if the array is not empty will not rewrite  
	else
		fifo->head++;           //increament the head
